...
A genus is eligible if the body satisfies ANY ONE of its rulesets (see genus_conditions.py --
rulesets are OR'd, fields within one ruleset are AND'd). Categorical fields (atmosphere,
body type, star type, volcanism) are hard gates within a ruleset. Temperature/gravity/pressure
are soft: confidence tapers from 1.0 inside a ruleset's documented range down to 0.0 over a
margin beyond either edge, since a transcribed range can't be trusted to the exact Kelvin/G --
a near-miss should read as lower confidence, not an identical hard fail. A genus's overall
confidence is the BEST (max) score across its matching rulesets, since each is an independent,
alternative path to eligibility, not a combined requirement.
"""
...
def predict_genera(entry:dict, nearest_star_type:str|None) -> list[tuple[str, float]]:
    ...
    for genus, rulesets in GENUS_RULESETS.items():
        ...
        for rs in rulesets:
            if not _hard_gates_pass(rs, nearest_star_type, planet_class, atmosphere_type, volcanism):
                continue
